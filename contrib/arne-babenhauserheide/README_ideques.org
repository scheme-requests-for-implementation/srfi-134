...
side is empty and the other has more than one element, but removing
elements is slower and has higher skitter, because it balances when
...
for removal could be linear if elements would be removed alternating
front and back.
...
Creating one ideque with a number with for-each allocates around 30 byte of memory.

Creating 1000000 ideques with map ideque allocates around 70 Bytes of memory per Ideque.
...
(fold (λ(number ideq) (ideque-add-front ideq number)) (ideque '()) (iota 1000000))
